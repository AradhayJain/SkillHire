...
const CustomTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    return (
      <div className="p-3 bg-white/80 backdrop-blur-sm shadow-lg rounded-lg border border-slate-200">
        <p className="label text-sm font-bold text-slate-800">{`${label}`}</p>
...
const AnalyticsPage = () => {
  ...
  return (
    <div className="space-y-8 bg-slate-50 p-4 sm:p-8 rounded-lg">
      {/* Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
        <div>
          <h1 className="text-3xl font-bold text-slate-800">Performance Analytics</h1>
          <p className="text-slate-500 mt-1">Track your job search progress and optimize your strategy.</p>
        </div>
        <select className="mt-4 sm:mt-0 px-4 py-2 bg-white border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div variants={cardVariants} initial="hidden" animate="visible" transition={{ delay: 0.4, duration: 0.5 }}>
          <Card className="p-6">
            <h3 className="text-lg font-semibold text-slate-800 mb-4">ATS Score Trend</h3>
            <ResponsiveContainer width="100%" height={300}>
              <AreaChart data={combinedChartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                ...
                <YAxis tick={{ fill: '#64748b', fontSize: 12 }} domain={[70, 'dataMax + 5']} />
        ...
        <motion.div variants={cardVariants} initial="hidden" animate="visible" transition={{ delay: 0.5, duration: 0.5 }}>
          <Card className="p-6">
            <h3 className="text-lg font-semibold text-slate-800 mb-4">Applications vs Interviews</h3>
             <ResponsiveContainer width="100%" height={300}>
        ...
        <motion.div variants={cardVariants} initial="hidden" animate="visible" transition={{ delay: 0.6, duration: 0.5 }}>
          <Card className="p-6 h-full">
            <h3 className="text-lg font-semibold text-slate-800 mb-4">Industry Distribution</h3>
          ...
          <Card className="p-6 h-full">
            ...
            <div className="space-y-4">
              {insights.map((insight) => (
                <div key={insight.title} className="flex items-start gap-4 p-4 rounded-lg bg-slate-100/70 border border-slate-200/80">
